...
{  
  ...
  //Initialize matrices  
    ...
    Keq[0][0]=5.07;  Keq[0][1]=2.83;  Keq[0][2]=6.76;  Keq[0][3]=3.77;

    L[0][0]=0.0828  ;   L[0][1]=0.003;
    L[1][0]=  0.0508 ;   L[1][1]= -0.0611;
    L[2][0]= 0.0243;   L[2][1]=0.1261;
    L[3][0]=0.3667;   L[3][1]=-0.1578;
...
digitalWrite(A3,HIGH);
    ...
    in3 = read_input3(-8, 1, 1);     // -12v -> 12v
    in4 = read_input4(-0.3, 0.2, 1); 
...
*/
  ...
  if (y_k[1][0] < 0.2 && y_k[1][0] > -0.1) //6&1
  {
    y_k[1][0] = 0; 
  }
...
Matrix.Multiply((float*)Keq, (float*)x_hat, 1, n, 1, (float*)v_k);



/*NONLINEAR Eqns*/
//f1=(mp*l*x_hat[3][0]*x_hat[3][0]+mp*g*sin(x_hat[2][0])*cos(x_hat[2][0]))/(Mc+mp*sin(x_hat[2][0])*sin(x_hat[2][0]));
//b1=1/(Mc+mp*sin(x_hat[2][0])*sin(x_hat[2][0]));
//f2=-(((mp+Mc)*g*sin(x_hat[2][0])+mp*l*x_hat[3][0]*x_hat[3][0]*sin(x_hat[2][0])*cos(x_hat[2][0]))/(l*(Mc+mp*sin(x_hat[2][0])*sin(x_hat[2][0]))));
//b2=-(cos(x_hat[2][0])/(l*(Mc+mp*sin(x_hat[2][0])*sin(x_hat[2][0]))));
//ueq=-((c*f2+f1+alpha*c*x_hat[3][0]+alpha*x_hat[1][0])/(c*b2+b1));
//alpha=-((f1+c*f2+(b1+c*b2)*ueq)/(x_hat[1][0]+c*x_hat[3][0]));
  ...
  Serial.print(x_dothat[0][0]   ); 
  Serial.print("uk:"); 
  Serial.print(u_k[0][0]);
  Serial.print(" xhat1veolcity=  ");    
  Serial.print(x_hat1[1][0],4);
  Serial.print("F1: ");
  Serial.print(f1);
...
//CL NL Input
  for (int i = 0; i < m; i++)
    {
      u_k[i][0]=(v_k[0][0]/cos(x_k[2][0]))+26.5*sin(x_k[2][0]);
    }
...
//DeadZone    
     if (u_k[0][0]>-21.2 && u_k[0][0]<-19)
       {
         out1=-20+0.1*u_k[0][0];
     }
    else if (u_k[0][0]<-14 && u_k[0][0]>-17)
       { 
          out1 = -16.7-0.2*u_k[0][0];
      }
     else
     {
      out1=u_k[0][0];
...
*/
  ...
  //write_outx(value, offset, gain, disp) 
   ...
   write_out1(out1, 18.7,  -0.5, 1);   // Pin 5  -12V to 12V
   //con2
